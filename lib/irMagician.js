    ...
    return new Promise((resolve, reject) => {
        SerialPort.list((err, ports) => {
            var validPorts = ports.filter(element => {
                ...
                if(/\/dev\/ttyACM[0-9]/.test(element.comName)){
                    return true    
                }
                return false
            })
...
function write(path) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            await setRecordPointer(json.data.length)
            await setPostScale(json.postscale)
            for (var key in array) {
                await setBank(array[key].bank, array[key].pos)
                await writeFragment(array[key].pos, json.data[key])
                    // console.log(key)
            }
            resolve()
        } catch(error) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            for (var key in array) {
                ...
                await setBank(array[key].bank, array[key].pos)
                var fragment = await dumpFragment(array[key].pos)
                data.push(fragment)
            }
...
exports.info = async port => {
    await autoSelectDevicePort()
        .then(validPort => {return port || validPort})
        .then(open).catch(error => {throw error})
    await info()
        .catch(error => {return error})
        .then(async res1 => {
            const res2 = await close().catch(error => {return error})
            if (res1 || res2) throw res1 || res2
        })
}

exports.version = async port => {
    await autoSelectDevicePort()
        .then(validPort => {return port || validPort})
        .then(open).catch(error => {throw error})
    await close().catch(error => {throw error})
}

exports.capture = async port => {
    await autoSelectDevicePort()
        .then(validPort => {return port || validPort})
        .then(open).catch(error => {throw error})
    await capture()
...
exports.play = async (path, port) => {
    await autoSelectDevicePort()
        .then(validPort => {return port || validPort})
        .then(open).catch(error => {throw error})
    await (async () => {if (path) await write(path)})()
        .then(play)
        .catch(error => {return error})
        .then(async res1 => {
            const res2 = await close().catch(error => {return error})
...
exports.dump = async (fileName, port) => {
    await autoSelectDevicePort()
        .then(validPort => {return port || validPort})
        .then(open).catch(error => {throw error})
    await getInfo()
        .then(async info => {await dump(info, fileName)})
        .catch(error => {return error})
        .then(async res1 => {
            const res2 = await close().catch(error => {return error})
            if (res1 || res2) throw res1 || res2
...
exports.reco = async value => {
    await autoSelectDevicePort()
        .then(validPort => {return port || validPort})
        .then(open).catch(error => {throw error})
    await setRecordPointer(value)
        .catch(error => {return error})
        .then(async res1 => {
            const res2 = await close().catch(error => {return error})
            if (res1 || res2) throw res1 || res2
